...
import { CalendarClock, Plus, Save, FlaskConical, Trash2 } from "lucide-react";
...
import {
  useLabAvailabilityQuery,
  useLabProfileQuery,
  useUpdateLabAvailabilityMutation,
...
const DRAFT_STORAGE_KEY = "lab-schedule-draft";

const LabSchedule = () => {
  const profileQuery = useLabProfileQuery();
  const availabilityQuery = useLabAvailabilityQuery();
  const updateAvailabilityMutation = useUpdateLabAvailabilityMutation();
  ...
  const labName = getDisplayName(profileQuery.data ?? {});
  const labSubtitle =
      ...
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Configure weekly time slots patients can book for lab visits.
          </p>
        </div>
